...
//
// Editor canónico: SlideOver lateral derecho (mismo patrón que
// RegistrarConsumoSlideOver, ServicesManager, etc.). Crear y editar
// comparten el mismo panel — el `mode` se infiere de la presencia de `id`.
//
...
export default function ProductsManager({ initial }: Props) {
  ...
  return (
    <div className="space-y-4">
        ...
        {items.map((p) => (
          ...
          >
            <button
              type="button"
              onClick={() => startEdit(p)}
              className="flex flex-1 items-center gap-3 min-w-0 text-left rounded-lg -m-1 p-1 hover:bg-overlay/40 transition-colors"
      ...
      {/* Editor — SlideOver lateral derecho (patrón canónico) */}
      <SlideOver
        open={!!draft}
        onClose={cancelDraft}
      ...
      >
        {draft && (
          // Cadena de altura canónica: flex min-h-0 flex-1 flex-col → body
          // flex-1 overflow-y-auto → footer shrink-0 sticky. Garantiza que
          // el botón Guardar sea siempre accesible aunque el form crezca.
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
              ...
              {/* Nombre */}
              <div>
                <label htmlFor="product-name" className="text-xs font-semibold text-ink-2 mb-1 block">
                  Nombre *
                </label>
                <input
                  id="product-name"
              ...
              {/* Descripción */}
              <div>
                <label htmlFor="product-desc" className="text-xs font-semibold text-ink-2 mb-1 block">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label htmlFor="product-price" className="text-xs font-semibold text-ink-2 mb-1 block">
                    Precio (€) *
                  </label>
                  <input
                    id="product-price"
                ...
                <div>
                  <label htmlFor="product-stock" className="text-xs font-semibold text-ink-2 mb-1 block">
                    Stock
                  </label>
                  <input
                    id="product-stock"
            ...
            {/* FOOTER sticky — Guardar siempre accesible */}
